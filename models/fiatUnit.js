...
const RateExtractors = Object.freeze({
  ...
  TRT: async ticker => {
    const api = new Frisbee({ baseURI: 'https://api.therocktrading.com' });
    const res = await api.get(`/v1/funds/BTC${ticker}/ticker`);
    if (res.err) throw new Error(`Could not update rate for ${ticker}: ${res.err}`);
    ...
    let rate = json?.last; // eslint-disable-line
    if (!rate) throw new Error(`Could not update rate for ${ticker}: data is wrong`);
...
export const FiatUnit = Object.freeze({
  EUR: { endPointKey: 'EUR', symbol: '€', locale: 'en-IE', source: FiatUnitSource.TRT },
  GBP: { endPointKey: 'GBP', symbol: '£', locale: 'en-IE', source: FiatUnitSource.TRT },
  USD: { endPointKey: 'USDC', symbol: '$', locale: 'en-IE', source: FiatUnitSource.TRT },
